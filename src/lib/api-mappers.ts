...
export function mapApiProductToFrontend(apiProduct: any): Product {
  ...
  return {
    ...
    shop: apiProduct.shop ? {
      ...
      customization: apiProduct.shop.customization || null,
      allowGuestCheckout: apiProduct.shop.allowGuestCheckout,
    } : undefined,
    location: apiProduct.location || apiProduct.shop?.location || '',
    distance: apiProduct.distanceKm != null ? `${Number(apiProduct.distanceKm).toFixed(1)} km` : apiProduct.area || apiProduct.distance || '',
...
export function mapApiServiceToFrontend(apiService: any): BeautyService {
  return {
    ...
    shop: apiService.shop ? {
      id: apiService.shop.id,
      name: apiService.shop.name,
      slug: apiService.shop.slug,
      logo: apiService.shop.logo,
      customization: apiService.shop.customization || null,
      allowGuestCheckout: apiService.shop.allowGuestCheckout,
    } : undefined,
...
export function mapApiAddressToFrontend(apiAddress: any): Address {
  return {
    ...
    latitude: apiAddress.latitude ?? null,
    longitude: apiAddress.longitude ?? null,
...
export function mapApiSellerVerificationToFrontend(apiVer: any): SellerVerification {
  return {
    ...
    fullName: apiVer.fullName,
    phoneNumber: apiVer.phoneNumber,
    shopId: apiVer.shopId,
    status: apiVer.status,
    type: apiVer.type,
    idType: apiVer.idType,
    idNumber: apiVer.idNumber,
    idFrontUrl: apiVer.idFrontUrl,
    idBackUrl: apiVer.idBackUrl,
    selfieUrl: apiVer.selfieUrl,
