...
export default function AdminUsersPage() {
  ...
  const toggleRole = async (userId: string, field: "isSeller" | "isRider" | "isAdmin") => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to ${action} ${field.replace("is", "")} access for ${user.name}?`,
      )
    )
      return;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="font-display text-2xl md:text-3xl font-bold text-warm-900">Customers</h1>
      ...
      {dataLoading ? (
        ...
        <Card className="p-12 text-center">
          ...
          <Button onClick={() => loadUsers(page, searchQuery, roleFilter)} className="mt-4">
            Retry
          </Button>
        </Card>
      ) : users.length === 0 ? (
              ...
              {userLoading ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <p className="text-xs font-medium text-warm-800/50 uppercase">
                      Access Controls
                    </p>
                    <div className="flex flex-wrap gap-2">
                      <Button
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {!selectedUser.banned && (
                        <Button
                          ...
                          variant="outline"
                          onClick={() =>
                            setActionDialog({ open: true, userId: selectedUser.id, action: "BAN" })
                          }
                        >
                          <Ban size={16} />
                          Ban
                  ...
                  {selectedUser.addresses && selectedUser.addresses.length > 0 && (
                    <div>
                      <p className="text-xs font-medium text-warm-800/50 uppercase mb-2">
                        Addresses
                      </p>
                      <div className="space-y-2">
          ...
          <div className="w-full max-w-md">
            <Card className="p-6">
              ...
              <p className="text-sm text-warm-800/60 mb-4">
                {actionDialog.action === "SUSPEND" &&
                  "This will suspend the user account. The user will not be able to access the platform."}
                {actionDialog.action === "BAN" &&
                  "This will permanently ban the user account. The user will no longer be able to use the platform."}
                {actionDialog.action === "REACTIVATE" &&
