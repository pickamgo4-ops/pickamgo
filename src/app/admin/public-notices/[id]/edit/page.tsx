...
export default function EditPublicNoticePage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    loadNotice();
  }, [authInitialized, authLoading, user, router, params.id]);

  const loadNotice = async () => {
    setLoading(true);
    try {
      const response = await api.get<{ notice: any }>(`/public-notices/${params.id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await api.patch(`/public-notices/${params.id}`, payload);
      if (response.success) {
  ...
  const handlePublish = async () => {
    try {
      await api.post(`/public-notices/${params.id}/publish`, {});
      router.push("/admin/public-notices");
    } catch {
      // ignore
    }
            ...
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-warm-800/60 mt-1">
                  Update notice content, targeting, and settings
                </p>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <select
                            ...
                            className="w-full rounded-xl border border-warm-200 bg-white px-3 py-2.5 text-sm"
                          >
                  ...
                  <Card className="p-6">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <label htmlFor="showCTA" className="text-sm text-warm-900">
                          Show CTA button
                        </label>
                      </div>
                      {form.showCTA && (
                        <>
                          ...
                          <div>
                            ...
                            <Input
                              ...
                              onValueChange={(v) => updateField("buttonUrl", v)}
                              placeholder="https://pickamgo.com/..."
                            />
                          </div>
                          <div>
                            <label className="block text-sm font-medium text-warm-900 mb-1">
                            ...
                            <select
                              ...
                              onChange={(e) => updateField("linkTarget", e.target.value)}
                              className="w-full rounded-xl border border-warm-200 bg-white px-3 py-2.5 text-sm"
                            >
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onChange={(e) => updateField("showAllPages", e.target.checked)}
                          className="rounded border-warm-300"
                        />
                        <label htmlFor="showAllPages" className="text-sm text-warm-900">
                          Show on all pages
                        </label>
                      </div>
                      {!form.showAllPages && (
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                          {pageTargets.map((target) => (
                            <label key={target.value} className="flex items-center gap-2 text-sm">
                              <input
                                type="checkbox"
                                checked={form.pageTargets.includes(target.value)}
                                onChange={() => togglePageTarget(target.value)}
                                className="rounded border-warm-300"
                              />
                              {target.label}
                            </label>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="rounded border-warm-300"
                        />
                        <label htmlFor="scheduleEnabled" className="text-sm text-warm-900">
                          Schedule notice
                        </label>
                      </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            ...
                            <Input
                              type="datetime-local"
                              value={form.startsAt}
                              onValueChange={(v) => updateField("startsAt", v)}
                            />
                          </div>
                          <div>
                            <label className="block text-sm font-medium text-warm-900 mb-1">
                            ...
                            <Input
                              type="datetime-local"
                              value={form.endsAt}
                              onValueChange={(v) => updateField("endsAt", v)}
                            />
                          </div>
                        </div>
                      )}
                ...
                <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex rounded-lg border border-warm-200 overflow-hidden">
                        <button
                          ...
                          className={`px-3 py-1.5 text-xs ${previewMode === "desktop" ? "bg-primary text-white" : "bg-white text-warm-800"}`}
                        >
                        ...
                        <button
                          ...
                          className={`px-3 py-1.5 text-xs ${previewMode === "mobile" ? "bg-primary text-white" : "bg-white text-warm-800"}`}
                        >
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onChange={(e) => updateField("isDismissible", e.target.checked)}
                          className="rounded border-warm-300"
                        />
                        <label htmlFor="isDismissible" className="text-sm text-warm-900">
                      ...
                      {form.isDismissible && (
                        <div className="flex items-center gap-2">
                          <input
                            type="checkbox"
                            id="rememberDismissal"
                            checked={form.rememberDismissal}
                            onChange={(e) => updateField("rememberDismissal", e.target.checked)}
                            className="rounded border-warm-300"
                          />
                          <label htmlFor="rememberDismissal" className="text-sm text-warm-900">
                            Remember dismissal
                          </label>
                        </div>
                      )}
                      <div>
                        ...
                        <Input
                          type="number"
                          value={String(form.reappearAfterHours)}
                          onValueChange={(v) =>
                            updateField("reappearAfterHours", parseInt(v) || 24)
                          }
                          min={0}
                        />
                      </div>
                  ...
                  <Card className="p-6">
                    ...
                    <div
                      className={`mx-auto border border-warm-200 rounded-lg overflow-hidden ${previewMode === "mobile" ? "max-w-sm" : "w-full"}`}
                    >
                      ...
                      <div className="p-4">
                        <div
                          className={`rounded-xl p-4 border-2 ${previewNotice.type === "WARNING" ? "bg-yellow-50 border-yellow-200" : previewNotice.type === "IMPORTANT" ? "bg-red-50 border-red-200" : previewNotice.type === "SUCCESS" ? "bg-green-50 border-green-200" : previewNotice.type === "MAINTENANCE" ? "bg-purple-50 border-purple-200" : previewNotice.type === "PROMOTION" ? "bg-pink-50 border-pink-200" : "bg-blue-50 border-blue-200"}`}
                        >
                          <div className="flex items-start justify-between gap-2">
                            <div className="flex items-start gap-2 flex-1">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-warm-800/70 mt-1 line-clamp-3">
                                  {previewNotice.message}
                                </p>
                                {form.showCTA && form.buttonText && (
                            ...
                            {form.isDismissible && (
                              <button
                                type="button"
                                className="text-warm-800/40 hover:text-warm-800/60"
                              >
                                <Eye size={14} />
                              </button>
                            )}
