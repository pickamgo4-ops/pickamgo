...
export default function EditAdminPromoPage() {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        setForm({
          ...
          minimumOrderAmount: String(p.minimumOrderAmount),
          fundingType: p.fundingType,
          usageLimit: p.usageLimit ? String(p.usageLimit) : '',
          usagePerCustomer: p.usagePerCustomer ? String(p.usagePerCustomer) : '0',
          startAt: p.startAt ? p.startAt.slice(0, 16) : '',
          endAt: p.endAt ? p.endAt.slice(0, 16) : '',
          status: p.status as Status,
          appliesToType: p.appliesTo ? JSON.parse(p.appliesTo).type : 'ALL',
          appliesToValues: p.appliesTo ? JSON.parse(p.appliesTo).values : [],
          customerEligibility: p.customerEligibility,
          discountAppliesTo: p.discountAppliesTo,
          campaignBudget: p.campaignBudget ? String(p.campaignBudget) : '',
        })
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await api.updateAdminPromo(params.id as string, payload)
      if (res.success) {
        setSuccess('Promo code updated successfully!')
        setTimeout(() => router.push('/admin/promo-codes'), 1000)
      } else {
        setError(res.error || 'Failed to update promo code')
      }
    } catch (err) {
      ...
      <main className="md:ml-64 min-w-0 max-w-full overflow-x-hidden pt-14 md:pt-0">
        <div className="max-w-4xl mx-auto min-w-0 p-4 md:p-6 lg:p-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="font-display text-2xl md:text-3xl font-bold text-warm-900">Edit Promo Code</h1>
              <p className="text-warm-800/60 text-sm mt-1">Update promotion settings</p>
            </div>
          </div>
