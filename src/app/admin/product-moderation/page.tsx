...
export default function ProductModerationPage() {
  ...
  const moderate = async (id: string, moderationStatus: "APPROVED" | "REJECTED") => {
    setSavingId(id);
    try {
      const response = await api.moderateProduct(id, {
        moderationStatus,
        moderationNotes: notes[id]?.trim() || undefined,
      });
      if (response.success) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="font-display text-2xl font-bold text-warm-900">Product Moderation</h1>
          <p className="text-sm text-warm-800/60">
            Review products before they appear in the marketplace.
          </p>
        </div>
      </div>
      ...
      {loadingProducts ? (
        ...
        <Card className="p-12 text-center">
          <CheckCircle className="mx-auto mb-3 text-emerald-500" size={42} />
          <p className="font-medium text-warm-900">No products awaiting review</p>
          <p className="mt-1 text-sm text-warm-800/60">The moderation queue is clear.</p>
        </Card>
      ) : (
              ...
              <div className="flex flex-col gap-4 lg:flex-row">
                <div className="flex min-w-0 flex-1 gap-4">
                  <img
                    ...
                    alt={product.name}
                    className="h-24 w-24 shrink-0 rounded-xl bg-warm-100 object-cover"
                  />
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <h2 className="font-semibold text-warm-900">{product.name}</h2>
                      <Badge variant="deal">Pending</Badge>
                    </div>
                    <p className="mt-1 text-sm text-warm-800/60">
                      {product.shop?.name || "Unknown shop"} ·{" "}
                      {product.seller?.name || product.seller?.email || "Unknown seller"}
                    </p>
                    <p className="mt-2 text-sm text-warm-800/80 line-clamp-3">
                      {product.description}
                    </p>
                ...
                <div className="w-full lg:max-w-sm">
                  <textarea
                    ...
                    rows={3}
                    className="w-full rounded-xl border border-warm-200 bg-transparent p-3 text-sm text-warm-900"
                  />
                  <div className="mt-2 flex flex-wrap justify-end gap-2">
                    <Button
                      ...
                      onClick={() => moderate(product.id, "REJECTED")}
                    ...
                    <Button
                      size="sm"
                      disabled={savingId === product.id}
                      onClick={() => moderate(product.id, "APPROVED")}
