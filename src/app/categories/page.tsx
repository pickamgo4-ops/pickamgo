...
export default function CategoriesPage() {
  ...
  return <div className="min-h-screen bg-warm-50 pb-20 md:pb-0"><Header /><main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8"><div className="mb-8"><h1 className="font-display text-3xl font-bold text-warm-900">All Categories</h1><p className="mt-2 text-warm-800/60">Explore products and services across PickAmGo.</p></div>{loading ? <div className="py-20 text-center text-warm-800/60">Loading categories...</div> : error ? <div className="rounded-2xl border border-red-200 bg-red-50 p-8 text-center"><p className="text-sm text-red-700">{error}</p><button type="button" onClick={loadCategories} className="mt-4 rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-white hover:bg-primary-dark">Try again</button></div> : categories.length === 0 ? <div className="rounded-2xl border border-warm-200 bg-white p-8 text-center text-warm-800/60">No categories available yet.</div> : <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{categories.map(category => { const Icon = getCategoryIcon(category); return <section key={category.id} className="rounded-2xl border border-warm-200 bg-white p-5 shadow-sm"><button type="button" onClick={() => router.push(`/discover?category=${encodeURIComponent(category.name)}`)} className="flex w-full items-center gap-3 text-left"><span className="flex h-12 w-12 items-center justify-center rounded-xl bg-primary/10 text-primary"><Icon size={22} strokeWidth={1.8} /></span><span className="min-w-0 flex-1"><strong className="block text-lg text-warm-900">{category.name}</strong><span className="text-xs text-warm-800/60">{category.count || 0} listings</span></span><ChevronRight size={18} className="text-warm-800/40" /></button>{category.children && category.children.length > 0 && <div className="mt-4 grid grid-cols-2 gap-2 border-t border-warm-100 pt-4">{category.children.map(child => <button key={child.id} type="button" onClick={() => router.push(`/discover?category=${encodeURIComponent(child.name)}`)} className="truncate rounded-lg bg-warm-50 px-3 py-2 text-left text-sm text-warm-800 hover:bg-primary/10 hover:text-primary">{child.name}</button>)}</div>}</section> })}</div>}</main><BottomNav /></div>
}