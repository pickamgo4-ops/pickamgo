...
import { Search, Package, CheckCircle, Clock, Truck, MapPin, ChevronLeft, Mail } from 'lucide-react'
    ...
    <div className="min-h-screen pb-20 md:pb-0">
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="font-display text-2xl md:text-3xl font-bold text-warm-900">Track Order</h1>
            <p className="text-warm-800/60">Enter your email and order number to track</p>
          </div>
        </div>
        ...
        {loading && (
          <div className="py-20 text-center text-warm-800/60">Tracking order...</div>
        )}

        {error && !loading && (
          <Card className="p-6 text-center">
            <Package size={48} className="text-red-300 mx-auto mb-3" />
            <h3 className="font-semibold text-warm-900">Order Not Found</h3>
            <p className="text-sm text-warm-800/60 mt-2">{error}</p>
          </Card>
        )}
            ...
            <Card className="p-6">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-sm text-warm-800/60">{trackingData.shopName ? `From ${trackingData.shopName}` : ''}</p>
                </div>
                <Badge variant="default">{trackingData.status.replace(/_/g, ' ')}</Badge>
              </div>
              ...
              <div className="space-y-3 mb-4">
                {trackingData.items.map((item: any, index: number) => (
                  <div key={index} className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-medium">GH₵{(Number(item.price) * item.quantity).toFixed(2)}</span>
                  </div>
                ))}
              </div>

              <div className="border-t border-warm-200 pt-3 flex justify-between font-bold">
                <span>Total</span>
                <span>GH₵{trackingData.total.toFixed(2)}</span>
              </div>
                ...
                {trackingData.timeline.map((step) => {
                  ...
                  return (
                    <div key={step.status} className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center ${step.completed ? 'bg-primary text-white' : 'bg-warm-100 text-warm-800/40'}`}>
                        <Icon size={18} />
                      </div>
                      <div>
                        <p className="text-sm font-medium text-warm-900">{step.label}</p>
                        {step.active && <p className="text-xs text-primary">Current status</p>}
                      </div>
                    </div>
                  )
                })}
