...
export default function CreateProductPage() {
  ...
  const uploadImage = () => {
    ...
    input.onchange = async () => {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || '/api'}/upload/image`, { method: 'POST', headers: { Authorization: `Bearer ${localStorage.getItem('token') || ''}` }, body })
      const data = await response.json()
      if (data.success) updateField('images', data.data.url)
