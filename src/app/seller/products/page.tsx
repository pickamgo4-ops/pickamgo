...
export default function SellerProductsPage() {
  ...
  const load = useCallback(async () => {
    ...
    else setError(productResponse.error || 'Failed to load products')
  ...
  const toggleVisibility = async (product: any) => {
    ...
    else setError(response.error || 'Could not update visibility')
  }
  ...
  const updateStock = async (id: string) => {
    ...
    if (response.success) {
      ...
      setError(response.error || 'Could not update stock')
    }
  }
  ...
  const archive = async (id: string) => {
    ...
    if (response.success) load()
    else setError(response.error || 'Could not delete product')
  }
    ...
    <SellerSidebar>
      <div className="space-y-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <h1 className="font-display text-3xl font-bold text-warm-900">Products</h1>
            <p className="text-warm-800/60 mt-1">Manage your catalog without changing orders.</p>
          </div>
          <Button onClick={() => window.location.href = '/seller/products/new'} icon={<Plus size={18} />}>
            Add Product
          </Button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="relative">
            <Search size={17} className="absolute left-3 top-1/2 -translate-y-1/2 text-warm-800/50" />
            <Input className="pl-9" placeholder="Search products" value={search} onChange={e => setSearch(e.target.value)} />
          </div>
          <select value={status} onChange={e => setStatus(e.target.value)} className="rounded-xl border border-warm-200 px-3 bg-white text-sm">
          ...
          <select value={categoryId} onChange={e => setCategoryId(e.target.value)} className="rounded-xl border border-warm-200 px-3 bg-white text-sm">
            <option value="">All categories</option>
            {categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
          <select value={sort} onChange={e => setSort(e.target.value)} className="rounded-xl border border-warm-200 px-3 bg-white text-sm">
        ...
        {products.length === 0 ? (
          <Card className="p-12 text-center">
            ...
            <p className="text-warm-800/60">No products match these filters.</p>
          </Card>
        ) : (
              ...
              <Card key={product.id} className="p-4">
                <div className="flex items-start gap-3 sm:gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <h2 className="font-semibold text-warm-900 truncate">{product.name}</h2>
                      <Badge variant={product.status === 'ACTIVE' ? 'verified' : product.status === 'HIDDEN' ? 'default' : 'deal'}>
                        {product.status}
                      </Badge>
                    </div>
                    <p className="text-sm text-warm-800/60 mt-1">{product.shopCategory?.name || product.category?.name || 'Uncategorized'}</p>
                  ...
                  <div className="flex items-center gap-1">
                    <button aria-label="Edit product" title="Edit product" onClick={() => window.location.href = `/seller/products/${product.id}/edit`} className="p-2 rounded-lg hover:bg-warm-100">
                      <Edit3 size={17} />
                    </button>
                    <button aria-label={product.status === 'ACTIVE' ? 'Hide product' : 'Show product'} title={product.status === 'ACTIVE' ? 'Hide product' : 'Show product'} onClick={() => toggleVisibility(product)} className="p-2 rounded-lg hover:bg-warm-100">
                ...
                {stockEditing === product.id && (
                  <div className="mt-3 flex items-center gap-2">
                    <Input type="number" value={stock} onValueChange={setStock} className="w-24" />
                    <Button size="sm" onClick={() => updateStock(product.id)}>Save</Button>
                    <Button size="sm" variant="ghost" onClick={() => setStockEditing(null)}>Cancel</Button>
                  </div>
                )}
              </Card>
            ))}
