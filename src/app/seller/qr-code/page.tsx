...
export default function SellerQrCodePage() {
  ...
  const load = async () => { const response = await api.getSellerQr(); if (response.success) setQr(response.data); setLoading(false) }
  useEffect(() => { load() }, [])
  const generate = async () => { const response = await api.generateSellerQr(); if (response.success) setQr(response.data) }
