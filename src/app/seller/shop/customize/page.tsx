...
const layouts: Array<{ id: ShopLayout; name: string; description: string }> = [
  ...
  { id: 'CAMPUS', name: 'Campus', description: 'Quick categories, popular items and deals' },
]
...
export default function SellerCustomizeShopPage() {
  ...
  const save = async (publish = false) => {
    ...
    if (publish) {
      ...
      setStatus(published.success ? 'Published' : published.error || 'Could not publish')
  ...
  return <SellerSidebar><div className="space-y-6">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="font-display text-3xl font-bold text-warm-900">Customize Shop</h1><p className="text-warm-800/60 mt-1">Build a storefront that feels like your business.</p></div><div className="flex gap-2"><Button variant="ghost" onClick={reset} icon={<RotateCcw size={16} />}>Reset</Button><Button variant="outline" onClick={() => window.open(`/shop/${shop.slug}`, '_blank')} icon={<ExternalLink size={16} />}>Open live shop</Button><Button onClick={() => save(false)} icon={<Save size={16} />}>Save Draft</Button><Button onClick={() => save(true)} icon={<Check size={16} />}>Publish</Button></div></div>
