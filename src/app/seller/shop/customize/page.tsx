...
export default function SellerCustomizeShopPage() {
  ...
  return <SellerSidebar><div className="space-y-6">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="font-display text-3xl font-bold text-warm-900">Customize Shop</h1><p className="text-warm-800/60 mt-1">Build a storefront that feels like your business.</p></div><div className="flex gap-2"><Button variant="ghost" onClick={reset} icon={<RotateCcw size={16} />}>Reset</Button><Button variant="outline" onClick={() => window.open(getShopUrl(shop.slug), '_blank')} icon={<ExternalLink size={16} />}>Preview Shop</Button><Button onClick={save} icon={<Save size={16} />}>Save Changes</Button></div></div>
    ...
    <div className="grid xl:grid-cols-[minmax(0,1fr)_420px] gap-6 items-start"><div className="space-y-6">
      ...
      <Card className="p-5"><h2 className="font-semibold text-warm-900 mb-4">Layout and content</h2><div className="grid sm:grid-cols-2 gap-3">{layouts.map(layout => <button key={layout.id} onClick={() => update({ layout: layout.id })} className={`text-left p-3 rounded-xl border-2 ${customization.layout === layout.id ? 'border-primary bg-primary/5' : 'border-warm-200'}`}><p className="font-semibold text-sm text-warm-900">{layout.name}</p><p className="text-xs text-warm-800/60 mt-1">{layout.description}</p></button>)}</div><div className="grid sm:grid-cols-3 gap-3 mt-4">{styleOptions.map(({ field, label, options }) => <label key={field} className="text-sm text-warm-800/70">{label}<select value={customization[field]} onChange={e => update({ [field]: e.target.value } as Partial<ShopCustomization>)} className="mt-2 w-full rounded-xl border border-warm-200 p-3 bg-white">{options.map(option => <option key={option.id} value={option.id}>{option.name}</option>)}</select></label>)}</div><label className="block mt-4 text-sm text-warm-800/70">Products per row: {customization.productColumns}<input type="range" min="2" max="5" value={customization.productColumns} onChange={e => update({ productColumns: Number(e.target.value) })} className="mt-2 w-full accent-primary" /></label><label className="block mt-4 text-sm text-warm-800/70">Featured product<select value={customization.featuredProductId || ''} onChange={e => update({ featuredProductId: e.target.value || null })} className="mt-2 w-full rounded-xl border border-warm-200 p-3 bg-white"><option value="">No featured product</option>{products.map(product => <option key={product.id} value={product.id}>{product.name}</option>)}</select></label><div className="grid sm:grid-cols-2 gap-3 mt-4">{[['showCategories','Categories'],['showFeatured','Featured product'],['showServices','Services'],['showReviews','Reviews'],['showAbout','About the shop'],['showHours','Opening hours'],['showContact','Contact information']].map(([field, label]) => <label key={field} className="flex items-center gap-2 text-sm text-warm-800/70"><input type="checkbox" checked={customization[field as keyof ShopCustomization] as boolean} onChange={e => update({ [field]: e.target.checked } as Partial<ShopCustomization>)} />{label}</label>)}</div></Card>
