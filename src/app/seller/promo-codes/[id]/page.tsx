...
import { ArrowLeft, Ticket, TrendingUp, DollarSign, ShoppingCart, Pause, Play, Trash2, Eye, Copy, CheckCircle, XCircle, Clock } from 'lucide-react'
...
export default function SellerPromoDetailPage() {
  ...
  const loadPromo = async (id: string) => {
    setLoading(true)
    try {
      const res = await api.getSellerPromo(id)
  ...
  const handleDelete = async () => {
    ...
    const res = await api.deleteSellerPromo(promo.id)
    if (res.success) {
      router.push('/seller/promo-codes')
    }
  }
    ...
    return (
      <SellerSidebar>
        <div className="text-center py-20">
          ...
          <Button className="mt-4" onClick={() => router.push('/seller/promo-codes')}>Back to Promos</Button>
        </div>
      </SellerSidebar>
    )
  }
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="outline" onClick={() => router.push(`/seller/promo-codes/${promo.id}/edit`)} icon={<XCircle size={18} />}>
              Edit
            </Button>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-warm-200">
            <p className="text-xs text-warm-800/60 font-medium mb-1">Usage</p>
            <p className="font-semibold text-warm-900">{promo.usageCount}{promo.usageLimit ? ` / ${promo.usageLimit}` : ''}</p>
          </div>
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-warm-200">
