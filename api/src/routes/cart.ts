...
router.post('/items', optionalAuthMiddleware, validateBody(createCartItemSchema), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (serviceId) {
      ...
      if (!userId && service.shop.allowGuestCheckout === false) {
        return errorResponse(res, 'This shop requires you to sign in before adding items to your cart.', 403, 'GUEST_CHECKOUT_REQUIRES_AUTH')
      }
    ...
    if (existingItem) {
      if (productId) {
        ...
        const variant = variantId ? await prisma.productVariant.findUnique({ where: { id: variantId }, select: { stock: true } }) : null
...
router.patch('/items/:id', optionalAuthMiddleware, validateBody(updateCartItemSchema), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (item.productId) {
      ...
      const availableStock = variant?.stock ?? product?.stock ?? 0
      if (quantity > availableStock) return errorResponse(res, 'Requested quantity exceeds available stock', 400)
    }
