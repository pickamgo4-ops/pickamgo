...
const customizationFields = {
  ...
  primaryColor: colorSchema.default('#1769D1'),
...
router.post('/:id/customization/reset', authMiddleware, requireRole(['SELLER']), async (req: AuthenticatedRequest, res) => {
  ...
  const customization = await prisma.shopCustomization.upsert({
    ...
    create: { shopId: result.shop!.id },
    update: {
      draftTheme: 'CLEAN', draftLayout: 'CLASSIC', draftPrimaryColor: '#1769D1',
...
router.get('/:slug', async (req, res) => {
  ...
  const shopResponse = {
    ...
    products: (shop.products || []).map((product: any) => ({
      ...product,
      shop: product.shop ? { ...product.shop, customization: effectiveCustomization(product.shop.customization) } : product.shop,
    })),
    services: (shop.services || []).map((service: any) => ({
      ...service,
      shop: service.shop ? { ...service.shop, customization: effectiveCustomization(service.shop.customization) } : service.shop,
    })),
    shopCategories: shop.shopCategories || [],
    customization: effectiveCustomization(shop.customization),
