  ...
  async (_req: AuthenticatedRequest, res) => {
    try {
      ...
      return res.status(500).json({
        ...
        diagnosis:
          "Verify Railway env vars, Cloudflare bucket permissions, and the public URL mapping before retrying uploads.",
      });
    }
  },
);
      ...
      const shop = await prisma.shop.findUnique({
        ...
        include: {
          ...
          orders: {
            take: 5,
            orderBy: { createdAt: "desc" },
            include: { customer: { select: { id: true, name: true } }, payment: true },
          },
          _count: { select: { products: true, services: true, orders: true } },
        },
      });
...
router.post(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      const { name, slug, description, image, emoji, color, parentId, displayOrder, isActive } =
        req.body;
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      if (online === "true") {
        ...
        where.OR = [{ isOnline: false }, { isAvailable: false }];
      }
      ...
      const [payouts, total] = await Promise.all([
        prisma.payout.findMany({
          ...
          include: {
            ...
            payoutMethod: {
              select: { provider: true, phoneNumber: true, accountName: true, type: true },
            },
          },
          orderBy: { createdAt: "desc" },
          skip: (page - 1) * limit,
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      return successResponse(
        res,
        { updated, total: shops.length },
        200,
        `Backfilled ${updated} shop slugs`,
      );
    } catch (error) {
...
router.post(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const { audiences, subject, html, text, selectedUserIds, testEmail, senderType } = parsed;
      ...
      if (audiences.includes("selected") && selectedUserIds && selectedUserIds.length > 0) {
        ...
        if (audiences.includes("buyers"))
          conditions.push({ isSeller: false, isRider: false, isAdmin: false });
        ...
        await Promise.allSettled(
          batch.map(async (user) => {
            ...
            const result = await sendEmail({
              ...
              text: text
                ? `${text.replace("{{name}}", user.name || "User")}\n\n---\nYou received this email because you have a PickAmGo account. To unsubscribe, reply to this email with "Unsubscribe" in the subject.`
                : undefined,
              purpose: senderType || "marketing",
            });
      ...
      const templates = [
        {
          ...
          body: "<p>Dear {{name}},</p><p>We have an important announcement to share with you.</p>",
        },
        {
          id: "promotion",
          name: "Promotion",
          subject: "Exclusive Offers Just for You!",
          body: "<p>Dear {{name}},</p><p>Check out our latest promotions and offers.</p>",
        },
        {
          id: "important",
          name: "Important Update",
          subject: "Important Update Regarding Your Account",
          body: "<p>Dear {{name}},</p><p>Please review the following important update.</p>",
        },
        {
          id: "maintenance",
          name: "Maintenance Notice",
          subject: "Scheduled Maintenance Notice",
          body: "<p>Dear {{name}},</p><p>Our platform will be undergoing scheduled maintenance.</p>",
        },
        {
          id: "seller",
          name: "Seller Announcement",
          subject: "Updates for PickAmGo Sellers",
          body: "<p>Dear {{name}},</p><p>Here are the latest updates for sellers on PickAmGo.</p>",
        },
        {
          id: "rider",
          name: "Rider Announcement",
          subject: "Updates for PickAmGo Riders",
          body: "<p>Dear {{name}},</p><p>Here are the latest updates for riders on PickAmGo.</p>",
        },
        {
          id: "buyer",
          name: "Buyer Announcement",
          subject: "Updates for PickAmGo Buyers",
          body: "<p>Dear {{name}},</p><p>Here are the latest updates for buyers on PickAmGo.</p>",
        },
      ];
